...
public class OauthHttpCalls {
    ...
    public static OauthBearerTokenJwt login(String clientId) {
        ...
        try {
            ...
            log.info("OAUTH_LOGIN_SERVER + OAUTH_LOGIN_ENDPOINT: {}", OAUTH_LOGIN_SERVER + OAUTH_LOGIN_ENDPOINT);
            log.info("postDataStr: {}", postDataStr);
            log.info("OAUTH_LOGIN_AUTHORIZATION: {}", OAUTH_LOGIN_AUTHORIZATION);
            if(OAUTH_WITH_SSL) {
                log.info("Call SSL -->");
                resp = doHttpsCall(OAUTH_LOGIN_SERVER + OAUTH_LOGIN_ENDPOINT, postDataStr, OAUTH_LOGIN_AUTHORIZATION);
            }else{
                log.info("Call Non SSL -->");
    ...
    public static OauthBearerTokenJwt introspectBearer(String accessToken){
        ...
        try {
            ...
            Map<String, Object> resp = null;
            log.info("OAUTH_INTROSPECT_SERVER + OAUTH_INTROSPECT_ENDPOINT: {}", OAUTH_INTROSPECT_SERVER + OAUTH_INTROSPECT_ENDPOINT);
            log.info("Token: {}", token);
            log.info("OAUTH_INTROSPECT_AUTHORIZATION: {}", OAUTH_INTROSPECT_AUTHORIZATION);
            if(OAUTH_WITH_SSL){
                log.info("Call SSL -->");
    ...
    private static Map<String, Object> doHttpCall(String urlStr, String postParameters, String oauthToken){
        try{
            ...
            log.error("at doHttpCall {}", e.getMessage());
        }
        return null;
    }
